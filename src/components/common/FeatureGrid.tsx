...
const FeatureGrid = ({ items, className = '' }: { items: FeatureItem[]; className?: string }) => {
  ...
  if( pathname?.includes('vnshblackbogo1') ) {
    return (
      <div className={`w-full ${className} ${pathname}`} >
        <div className={`max-w-7xl mx-auto px-4 ml-[-20px] mr-[-20px] md:ml-[10px] md:mr-[10px] mb-[7px]`}>
          <div className={`grid grid-cols-1 md:grid-cols-1 lg:grid-cols-3 gap-4`}>
            {items.map((item, index) => (
              <div key={index} className="flex flex-col items-center text-center">
                ...
                <p className="text-[16px] font-[800] mb-4 text-[#212529]">{item.title}</p>
